...
void DumpResults(StunClientLogicConfig& config, StunClientResults& results)
{
    ...
    // 基于行为和过滤测试结果判断NAT类型
    int natType = 4; // 默认未知
    std::string natTypeStr = "Unknown NAT Type"; // 默认未知类型



    if (results.behavior == EndpointIndependentMapping && results.filtering == EndpointIndependentFiltering) {
        natType = 0; // Full Cone NAT
        natTypeStr = "Full Cone NAT";
    } else if (results.behavior == EndpointIndependentMapping && results.filtering == AddressDependentFiltering) {
        natType = 1; // Restricted Cone NAT
        natTypeStr = "Restricted Cone NAT";
    } else if (results.behavior == EndpointIndependentMapping && results.filtering == AddressAndPortDependentFiltering) {
        natType = 2; // Port Restricted Cone NAT
        natTypeStr = "Port Restricted Cone NAT";
    } else if (results.behavior == AddressDependentMapping || results.behavior == AddressAndPortDependentMapping) {
        natType = 3; // Symmetric NAT
        natTypeStr = "Symmetric NAT";
    }

    // 输出NAT类型和外部IP/端口
    Logging::LogMsg(LL_ALWAYS, "NAT Type: %d", natType);
    Logging::LogMsg(LL_ALWAYS, "NAT Type: %s", natTypeStr.c_str());
    results.addrMapped.ToStringBuffer(szBuffer, buffersize);
    Logging::LogMsg(LL_ALWAYS, "External IP: %s", szBuffer);
    Logging::LogMsg(LL_ALWAYS, "External Port: %d", results.addrMapped.GetPort());
